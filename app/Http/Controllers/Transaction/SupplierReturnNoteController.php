...
class SupplierReturnNoteController extends Controller
{
    ...
    public function getUnsavedSessions()
    {
        try {
            ...
            foreach ($unsavedSessions as $doc_no) {
                $cacheKey = 'grn_loaded_srn_' . $doc_no;

                $grnMetadata = Cache::get($cacheKey);

                if (!$grnMetadata) {
                    $filteredSessions[] = $doc_no;
                } else {
                    // This is a GRN-loaded session, clean it up if it's old (more than 10 minutes)
                    $loadedTime = Carbon::parse($grnMetadata['loaded_at']);
    ...
    public function cleanupSRNSession($srnNumber)
    {
        try {
            // Delete all temp products for this GRN
            $deletedCount = TempTransactionDetail::where('doc_no', $srnNumber)
                ->where('temp_transaction_header_id', 0)
                ->delete();

            // Also delete temp header if exists
            TempTransactionHeader::where('doc_no', $srnNumber)->delete();

            return response()->json([
                'success' => true,
                'message' => 'SRN session cleaned up successfully.',
                'deleted_count' => $deletedCount
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'success' => false,
                'message' => 'Failed to cleanup SRN session.',
                'error' => $e->getMessage()
            ], 500);
        }
    }
    ...
    public function store(TempTransactionHeaderRequest $request)
    {
        try {
            return DB::transaction(function () use ($request) {
                ...
                $srnNumber = DocNumber::generate('SRN', 'SRN', 8, $data['location']);
                ...
                return response()->json([
                    'success' => true,
                    'message' => 'Supplier return note stored successfully.',
            ...
            return response()->json([
                'success' => false,
                'message' => 'Failed to store supplier return note.',
                'error'   => $e->getMessage(),
            ], 500);
        }
    }
